...
#model = load_model('CBA_loss_0.0084.h5')
model = load_model('CBAF_0.0070.h5')
...
mae_after_destd_1st_hour = np.mean(np.abs((destandardized_predictions_1st_hour.squeeze()) - data[sequence_length :-1, -2]))
...
mae_after_destd_2nd_hour = np.mean(np.abs((destandardized_predictions_2nd_hour.squeeze()) - data[sequence_length + 1:, -2]))
...
actual_price_changes_1st_hour = np.sign(data[sequence_length + 1:, -2] - data[sequence_length :-1, -2])
predicted_price_changes_1st_hour = np.sign(
    np.squeeze(destandardized_predictions_1st_hour) - data[sequence_length :-1, -2]
)

# Calculate accuracy for price movement direction prediction for the second hour
actual_price_changes_2nd_hour = np.sign(data[sequence_length + 2:, -2] - data[sequence_length + 1:-1, -2])
predicted_price_changes_2nd_hour = np.sign(
    np.squeeze(destandardized_predictions_2nd_hour) - data[sequence_length + 1:, -2]
)
...
absolute_price_changes_1st_hour = np.abs(data[sequence_length + 1:, -2] - data[sequence_length :-1, -2])
...
actual_price_changes_2nd_hour = np.sign(data[sequence_length + 2 :, -2] - data[sequence_length + 1 : -1, -2])
predicted_price_changes_2nd_hour = np.sign(
    np.squeeze(destandardized_predictions_2nd_hour[:-1]) - data[sequence_length + 1  :-1, -2]
)
...
absolute_price_changes_2nd_hour = np.abs(data[sequence_length + 2 : , -2] - data[sequence_length + 1 : -1, -2])
...
plt.plot(data[sequence_length + 2:-1, -2], label='Actual close price', alpha=0.7)
